...
const UserProfile = () => {
  ...
  const loadProfileImage = () => {
    ...
    let savedImage = localStorage.getItem(key);

    // ✅ If no image → assign default properly
    if (!savedImage) {
      savedImage = "/default-avatar.png";

      // store default for THIS USER
      localStorage.setItem(key, savedImage);
    }
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.1.17:8082/api/users/me", {
      method: "GET",
      headers: {
  ...
  const handleImageChange = (e) => {
    ...
    reader.onloadend = () => {
      ...
      console.log("Base64:", base64Image?.slice(0, 50)); // 🔍 DEBUG
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch("http://192.168.1.17:8082/api/users/me", {
        method: "PUT",
        headers: {
          Authorization: "Bearer " + token,
          "Content-Type": "application/json",
